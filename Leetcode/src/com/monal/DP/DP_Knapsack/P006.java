  ...
  class Solution {
    ...
    public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
      ...
      for (List<Integer> offer : special) {
        ...
        if (offerPrice < individualPrice)
          validOffers.add(offer);
      }

      return dfs(price, validOffers, needs);
    }
    ...
    private int dfs(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
      ...
      // Base case
      boolean allZero = needs.stream().allMatch(n -> n == 0);
      if (allZero) {
        memo.put(new ArrayList<>(needs), 0);
        return 0;
      }
      ...
      for (List<Integer> offer : special) {
        ...
        if (canUse) {
          int offerPrice = offer.get(offer.size() - 1);
          int costWithOffer = offerPrice + dfs(price, special, newNeeds);
