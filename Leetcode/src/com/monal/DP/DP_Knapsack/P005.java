...
public class P005 {
  ...
  private static int coinChangeMemo(int[] coins, int n, int amt, int[][] memo) {
    ...
    int include = coinChangeMemo(coins, n, amt - coins[n - 1], memo); // no n-1, because we can reuse the coin
  ...
  private static int coinChangeTab(int[] coins, int n, int amt) {
    ...
    for (int i = 0; i <= n; i++) dp[i][0] = 1; // 1 way to make 0 amount (use nothing)
  ...
  public static int minCoins(int coins[], int amount) {
    int[][] memo = new int[coins.length + 1][amount + 1];
    for (int m = 0; m < coins.length + 1; m++) Arrays.fill(memo[m], -1);
  ...
  private static int minCoinsMemoized(int[] coins, int n, int amt, int[][] memo) {
    ...
    // If current coin value is greater than remaining amount, skip it
    if (coins[n - 1] > amt) return memo[n][amt] = minCoinsMemoized(coins, n - 1, amt, memo);
