...
public class P002 {
  ...
  @SuppressWarnings("unused")
  private int robSpaceOptimized(int[] money) {
    int n = money.length;
    // we only need the last two values of dp - if we have index i, we only need
    // dp[i-1] and dp[i-2]
    int prev1 = 0;
    int prev2 = money[0];

    for (int i = 2; i <= n; i++) {
      int curr = Math.max(money[i - 1] + prev1, prev2);
      prev1 = prev2;
      prev2 = curr;
    }
  ...
  @SuppressWarnings("unused")
  private int robblerIISpaceOptimized(int arr[]) {
    int n = arr.length;
    if (n == 1)
      return arr[0];
    if (n == 2)
    ...
    // Case 1: Rob the first house and skip the last one
    int prev_1 = arr[0];
    int prev_2 = Math.max(arr[0], arr[1]);
    for (int i = 2; i <= n - 2; i++) {
      int curr = Math.max(arr[i] + prev_1, prev_2);
      prev_1 = prev_2;
      prev_2 = curr;
    }

    // Case 2: Skip the first house and rob the last one
    int prev_3 = 0; // not robbing the first house
    int prev_4 = arr[1];
    for (int i = 2; i <= n - 1; i++) {
      int curr = Math.max(arr[i] + prev_3, prev_4);
      prev_3 = prev_4;
      prev_4 = curr;
    }

    // Return the maximum of both cases
    return Math.max(prev_2, prev_4);
  }
