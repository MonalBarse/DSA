 ...
 * - Key insight: Instead of checking all subarrays (O(n²)), we slide a window (O(n))
 */
public class SlidingWindow {
    ...
    public static int countAnagrams(String text, String pattern) {
        ...
        if (windowMap.equals(patternMap)) count++; // .equals checks if both maps have same keys and values
     ...
     * - You have been given a string and an integer. You have to find the length of the longest
      ...
      int n = s.length();
        ...
        for (int t = 1; t <= 26; t++) {
            ...
            while (end < n) {
                ...
                if (count[s.charAt(end) - 'a']++ == 0) distinct++;
                if (count[s.charAt(end) - 'a'] == k) atLeastK++;
                end++;
                // shrink while too many distinct
                while (distinct > t) {
                    if (count[s.charAt(start) - 'a']-- == k) atLeastK--;
                    if (count[s.charAt(start) - 'a'] == 0) distinct--;
                    start++;
                }
     ...
     * minimum (contiguous) substring ‘w’ of ‘s’, such that ‘t’ is a subsequence of ‘w'.
     * A subsequence is a sequence that can be derived from another sequence by
     * removing zero or more elements, without changing the order. While, a substring is a contiguous part of a string.
     */
    ...
    public static String minWindowSubsequence(String s, String t) {
        ...
        for (int i = 0; i < sLen; i++) {
            if (s.charAt(i) != t.charAt(0)) continue; // skip if current char doesn't match t's first char
            ...
            if (tPtr == tLen) {
                ...
                while (tPtr >= 0) {
                    if (s.charAt(sPtr - 1) == t.charAt(tPtr)) tPtr--;
                    sPtr--;
                }
     ...
     * - Subarrays with at most K-1 distinct integers = subArrayWithAtMostK(k - 1, arr)
     * - Result = Total subarrays - Subarrays with at most K-1 distinct integers
     */
     ...
     * - Key insight: subarrays with exactly K = subarrays with at most K - subarrays with at most K-1
     ...
     *   This is becuase we can replace all other characters with the most frequent character
     *   for example in "AABABBA" with k = 2, window "AABABBA" has max freq of B = 3, A = 4
     *
     ...
     * Given an integer array cardPoints and an integer k, return the maximum score you can obtain by
...
/*
    ...
    // TEMPLATE 4: Count Valid Windows - i.e for eg "Count of subarrays with sum <= target"
...
👉 You're either dealing with a **fixed-size window** or a **variable-size window**.
Understanding *which flavor you’re in* is your first and most important insight.
...
You're being asked to **slide a window of size `k` across the array** and compute something at every step.
...
These problems are about **stretching or shrinking the window** based on some condition. Examples:

* "Longest substring with at most K distinct characters"
* "Smallest subarray with sum ≥ target"
* "Longest substring with all same characters after at most K replacements"

Here, the **window size is not fixed** — it grows or shrinks to satisfy a condition.
...
* At each step, check whether the window is valid, and update result accordingly
...
The `left` and `right` pointers are your **sliding frame**. Together they define your current candidate window.

In **fixed-size**, you mostly **control `right`** and calculate `left` indirectly.
...
Because this setup ensures **O(n)** processing — every element is added and removed at most once.
...
* Keep the “most important” elements (like max, min, or first negative) at the front
* Discard irrelevant ones from the back as you move forward

You’re essentially maintaining a **monotonic queue** — one that remains sorted by importance.

---

### ✨ Mathematical Insights: "At Most K" Trick

Sometimes, rather than tracking exact counts, you can **transform the question**.
...
These techniques work because **prefix sums** of counts give you powerful tools for slicing the data differently.

So instead of brute-forcing, you break a hard problem into two easier ones and subtract their results.

This is **mathematical laziness at its best** — you reduce logic, not precision.
...
* Sliding window = You are managing a **stretchy window** that walks through the array.
...
* You apply **math or count tricks** to convert indirect problems into direct ones.

If you can learn to “see” the shape of the problem (fixed-size, variable-size, tracking max/min, uniqueness, frequency…), then you’ll instinctively know:
